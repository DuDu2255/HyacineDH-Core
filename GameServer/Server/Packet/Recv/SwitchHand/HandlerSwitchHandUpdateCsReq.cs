...
public class HandlerSwitchHandUpdateCsReq : Handler
{
    public override async Task OnHandle(Connection connection, byte[] header, byte[] data)
    {
        ...
        var info = component.UpdateHandInfo(req.HKAEEMABJLA ?? new GODHDEIPDJL());
        if (info.Item2 == null)
            await connection.SendPacket(new PacketSwitchHandUpdateScRsp(info.Item1, req.HKAEEMABJLA, req.CNPILGNBDNB));
        else
            await connection.SendPacket(new PacketSwitchHandUpdateScRsp(info.Item2, req.HKAEEMABJLA, req.CNPILGNBDNB));
    }
}
